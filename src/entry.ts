...
// Enrty manages a key/value pair and it's subscribers
export class Enrty<T> {
  // Entry key
  private key: string;
  // Entry value
  private val: T | null;
  // Enrty-level subscribers
  ...
  // Get will reutrn the Entry value
  Get(): T | null {
    return this.val;
  }

  // Put will set the Enrty value
  Put(val: T) {
    if (this.closed) {
      return;
    }
