...
export class Indexer {
  ...
  // Get will get the cuurent index value (and increment for the next use)
  Get(): string {
...
export class Subber<T> {
  ...
  Signal(key: string, value: T | null) {
    ...
    this.ForEach((sKey: string, sfn: SubFn<T>) => {
      if (sfn(key, value) === true) {
        // Subscrbing function returned true, unsubscribe
      }
    });
  }
  ...
  Sub(fn: SubFn<T>, subKey?: string): string {
    ...
    // Set funciotn for newly created subKey
    this.m[subKey] = fn;
  ...
  Close() {
    ...
    // TODO: Investigate if this will leak, if so - manually de-referense to help the GC
    this.m = NewSubMap();
  }
}
